...
function DeepMask:createMaskBranch(config)
  ...
  local attentionBranch = nn.Sequential():add(self.attentionBranch)
  attentionBranch:add(nn.Replicate(128, 2))

  maskBranch:add(nn.ParallelTable():add(nn.Identity()):add(attentionBranch))
  ...
  if config.gSz > config.oSz then
    local maskOutput = nn.Sequential()
    maskOutput:add(nn.Copy('torch.CudaTensor', 'torch.FloatTensor'))
    maskOutput:add(nn.View(config.batch, config.oSz, config.oSz))
    maskOutput:add(nn.SpatialReSamplingEx{owidth=config.gSz, oheight=config.gSz,
                  mode='bilinear'})
    maskOutput:add(nn.View(config.batch, config.gSz*config.gSz))
    maskOutput:add(nn.Copy('torch.FloatTensor', 'torch.CudaTensor'))
    self.maskBranch:add(maskOutput)
  end
